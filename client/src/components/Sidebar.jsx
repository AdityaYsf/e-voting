...
function NavItem({ label, icon, path, onNavigate }) {
	if (!path) {
		return (
				...
				<>
					<span
						className={[
							...
							isActive
								? "h-[60%] scale-y-100"
								: "h-[60%] scale-y-0 group-hover:scale-y-100",
						].join(" ")}
					/>
					<span
						className={[
							...
							isActive
								? "bg-white/[.16]"
								: "bg-white/[.08] group-hover:bg-white/[.16]",
						].join(" ")}
					>
						<Icon name={icon} size={14} stroke="currentColor" />
					</span>
...
function SidebarContent({ onNavigate }) {
	return (
		<aside className="relative flex flex-col w-[228px] bg-[#082759] h-full overflow-y-auto border-r border-white/[.06] sb-shimmer">
			{/* Brand */}
			<div className="relative px-4.5 pt-5 pb-4 border-b border-white/[.08]">
				<div className="flex items-center gap-3 mb-3.5">
					...
					<div>
						<div className="text-[15px] font-extrabold text-white leading-tight tracking-[-0.01em]">
							e-Pemilihan
						</div>
						<div className="text-[10px] text-white/40 mt-0.5 tracking-[.06em] uppercase">
							Portal Warga · Akses Publik
						</div>
					</div>
				</div>
...
export default function Sidebar({ isOpen, onClose }) {
	...
	return (
		<>
			...
			<div className="hidden md:block sticky top-0 h-screen animate-slide-in-left z-20">
				<SidebarContent />
			</div>
			...
			>
				...
				<button
					onClick={onClose}
					className="absolute top-3 right-[-40px] z-10 w-8 h-8 flex items-center justify-center rounded-full bg-white/20 hover:bg-white/30 text-white text-xl transition-colors"
