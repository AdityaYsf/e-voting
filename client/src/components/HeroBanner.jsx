export default function HeroBanner({ countdown }) {
	return (
		<div className="relative flex flex-col md:flex-row md:items-center md:justify-between bg-gradient-to-br from-[#082759] via-[#0D3A7A] to-[#1558B0] rounded-xl px-5 md:px-8 py-6 md:py-7 mb-5 overflow-hidden shadow-blue animate-fade-slide-down gap-5 md:gap-0">
			{/* Animated blobs */}
			...
			<div className="relative z-[2]">
				...
				<h1 className="text-[18px] md:text-[22px] font-extrabold text-white tracking-[-0.03em] mb-1.5 leading-tight">
				...
				{/* Progress bar */}
				<div className="mt-3.5 w-full md:w-[200px]">
					...
					<div className="h-[5px] bg-white/[.12] rounded-full overflow-hidden">
						<div
							...
							style={{
								width: "58.9%",
								boxShadow: "0 0 8px rgba(103,232,249,.5)",
							}}
						/>
					</div>
				</div>
			...
			{/* Right — countdown */}
			{/* On mobile: left-aligned row, on desktop: right-aligned column */}
			<div className="relative z-[2] flex flex-row md:flex-col items-center md:items-end gap-3 md:gap-0 md:text-right flex-shrink-0">
				<div>
					<div className="text-[10px] text-white/40 uppercase tracking-[.1em] mb-1 md:mb-1.5">
						Waktu Tersisa
					</div>
					<div
						className="font-mono text-[28px] md:text-[36px] font-medium text-white tracking-[3px] leading-none"
						style={{ textShadow: "0 2px 12px rgba(0,0,0,.2)" }}
					>
						{countdown}
					</div>
					<div className="text-[10px] text-white/32 mt-1 md:mt-1.5">
						Penutupan pukul 17.00 WIB
					</div>
				</div>
			</div>
		</div>
